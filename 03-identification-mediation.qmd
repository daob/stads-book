...
The simulation builds the world; the two `sem()` calls fit the rival stories; the last line prints their fit statistics. They are identical to every decimal, in this dataset and in every dataset either model will ever meet.
...
So far, demolition. The constructive turn begins by drawing the confounding into the model. Connect the two disturbances with a curved arrow and give it a parameter, $\rho$, the correlation between everything-else-about-$M$ and everything-else-about-$Y$.
...
Count, as @saris1984 taught. With the direct path included, the unknowns are $a$, $b$, $c$, and $\rho$: four parameters facing three observable correlations. One short; not identified, and this time the counting rule alone convicts. But suppose the theory can credibly assert that there is *no direct path*: $X$ touches $Y$ only through $M$. Then the parameters are three, and the tracing rules give three equations:
