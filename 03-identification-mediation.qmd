...
<!-- DO: make sure the notation coincides with the new version of chapter 2. Also remember the "tracing rules" are implemented as the decomposition rule. Check the entire chapter below against these points. -->
$$
\begin{aligned}
M &= a\,X + \zeta_M \\
Y &= b\,M + c\,X + \zeta_Y .
...
<!-- DO: include an inline exercise that asks the reader to change some of the parameter values above. They should observe that the chisquare changes but remains identical between the two models, confirming that these models are equivalent for all parameter values. -->
...
If we believe there is unmeasured confounding then this should be reflected in a disturbance term correlation. 
<!-- DO: I absolutely hate these imperative openers: "Count, the way @saris1984 taught you to.". Who are you talking to? I also hate your overuse of useless and immediately discarded metaphors. And your exaggerations (eg "disaster"). Check all your writing for more of this awful stuff. --> 
...
<!-- DO: ensure consistent notation (just use LISREL notation) is used from ch2 and throughout this chapter, also in the figures. -->
...
The middle equation says the naive mediator-outcome correlation is contaminated: it contains $b$ plus a contribution from the correlated disturbances. Now look at the other two. Neither contains $\psi$ at all, because no valid trace from $X$ runs through the curved arrow. So we can easily solve for our target estimand of interest, beta by dividing the two equations:
...
@sec-rq-to-interpretation already did this division by hand. The Oregon arithmetic, where an effect on those drawn was divided by a 25-point rise in coverage, is $\rho_{XY}/\rho_{XM}$ in different clothes. The numerator has a name of its own, the **intention-to-treat** effect, and the assumption that licensed the division was the exclusion restriction all along.
...
<!-- DO: rewrite the sentence below in more normal language -->
...
<!-- DO: turn this bit below into an inline exercise. Do not assume too many programming skills (some are okay but pmax(pmin( is too difficult. -->
```{r}
...
<!-- DO: include a few high-profile economics examples, especially about policy impact evaluation. It is okay to mention classics, but also look for more recent examples in high-impact journals. -->
...
<!-- DO: expand this section a bit by defining both these quantities clearly mathematically, and working through a numerical example where they give a different answer than the linear model -->
The two add up to the total effect by construction, a decomposition as tidy as the linear one, and in the linear model without interactions the NIE is exactly $ab$. With interactions or a logistic outcome the classic formulas and the counterfactual ones part ways, and the counterfactual ones are right.
...
A closing word on standard errors, because the mediation literature is oddly obsessed with them. The indirect effect $ab$ is a product of two estimates, and lavaan's `:=` operator gives its standard error by the delta method in one line. Bootstrapping, which is discussed in subsequent chapters, gives nearly the same answer at ordinary sample sizes.^[The product of two normal estimates is not itself normal, which matters near zero and in small samples [@mackinnon2007]; the bootstrap handles this and is a fine tool.]
