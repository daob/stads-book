...
In the north of the Netherlands, decades of gas extraction have caused earthquakes, and the people who live above the gas field vote differently from the rest of the country. Researchers surveying the region proposed a chain of explanations: experiencing earthquakes erodes trust in the national government, feeds a sense of injustice, and strengthens regional identification, and each of these in turn pushes people toward a protest vote [@otjes2020]. Note the shape of the claim. It does not stop at "earthquakes cause protest voting"; it says the effect runs *through* something. Questions of this shape, called **mediation** questions, are among the most asked in the social and health sciences, because a mechanism is what a theory is made of, and because a mediator suggests a lever: if distrust carries the effect, perhaps restoring trust can interrupt it.

This chapter uses mediation as a case study in something larger. The previous chapter ended with a warning that a model which fits the data is not thereby true. Here we meet the hard version of that warning. For some questions, no amount of data can distinguish the truth from a rival story, because the design never put the answer into the data in the first place. Statisticians call this the problem of "identification". The word sounds bureaucratic; what it names is a limit on knowledge, and the design, not the sample size, sets the limit.

## The mediation model

By the recipe of @sec-theory-to-glm, the simplest mediation theory becomes two equations. Write $X$ for the cause, $M$ for the mediator, $Y$ for the outcome, all standardized:
...
The tracing rules hand us the decomposition that makes mediation attractive. Two traces connect $X$ and $Y$: the direct path, contributing $c$, and the path through the mediator, contributing $a \times b$. Their sum, the model-implied correlation, is called the **total effect**; $ab$ is the **indirect effect**; $c$ the **direct effect**. Estimate the equations, multiply, and you have "the share of the effect that runs through the mediator".^[This estimation recipe has circulated in psychology for decades under the name of @baron1986. The recipe is not our subject; what the numbers mean is.]
...
Indirect $= 0.5 \times 0.4 = 0.20$; direct $= 0.10$; total $= 0.30$; proportion mediated $= 0.20/0.30 = 2/3$. The claim: two thirds of education's benefit for longevity comes about because education raises health knowledge, which raises longevity, so that an intervention on health knowledge alone could capture most of the benefit. Feel the size of that claim. The rest of this chapter asks what entitles anyone to it.
:::

## The assumptions behind the arrows

Equations [-@eq-mediation] are claims, and by now you know where their credibility must come from. Reading $\hat a$, $\hat b$, and $\hat c$ causally requires, beyond linearity and additivity: no confounding of $X$ and $M$; no confounding of $M$ and $Y$, which is to say $\zeta_M$ and $\zeta_Y$ are uncorrelated; and the correct causal order, with the arrows pointing the way drawn.

Here is mediation's trap. Randomizing $X$, the strongest design feature we own, buys the first assumption completely, since a coin flip has no common causes with anything. It buys nothing for the other two. Nobody randomized the mediator. Within each arm of a randomized experiment, the people with high mediator values are simply different people from those with low values, and whatever makes them different may drive the outcome on its own. The $b$ path is an observational study smuggled inside the experiment. Methodologists have said this plainly for a long time: the mediator-outcome assumption "is needed even in the randomized trial context because, in a trial, although the exposure has been randomized, the mediator typically has not been" [@vanderweele2016]. In one randomized trial of cognitive behavioral therapy, a naive mediation analysis of antidepressant use made the antidepressants look harmful; VanderWeele's verdict is that "we get nonsense from the traditional approach if we ignore mediator-outcome confounding".

Instead of warning you further, let me have you watch the failures happen. Twice.

## Failure one: the arrows can be backwards

We simulate a world where the mediation model is exactly true. The cause raises the mediator ($a = 0.5$), the mediator raises the outcome ($b = 0.7$), and there is no direct effect. Then we fit two models: the true chain, and the same chain with every arrow reversed.
...
The simulation builds the world; the two `sem()` calls fit the rival stories; the last line prints their fit statistics. They are identical. Not similar: identical to every decimal, in this dataset and in every dataset either model will ever meet.
...
This situation has a name. Two models, or two parameter values, are **not identified** when they imply the same distribution of observable data. The data then have no opinion, however many of them you collect. @saris1984 supply the accounting perspective: a model with more parameters than observed variances and covariances cannot possibly be identified, but the counting rule is only a necessary condition, and the reversed chain passes it while the direction stays unidentifiable all the same. What can break the tie is design. If $x_1$ was randomized, an arrow into $x_1$ is absurd, because nothing in the system causes a coin flip; the reversed chain dies by design, not by fit. The earthquake study leans on the same logic in weaker form: earthquakes shake houses, opinions do not shake the ground, so the causal order of $X$ is secure. For the mediator-outcome pair, nothing is secure. Distrust may drive protest votes, or a protest identity may feed distrust, and the survey cannot tell.

## Failure two: the hidden confounder

Now the second assumption. In a new world, the cause still raises the mediator ($a = 0.5$), but the mediator does nothing whatsoever to the outcome. Instead, an unmeasured variable $U$ drives both mediator and outcome. Think of a randomized encouragement to study ($X$), study time ($M$), exam scores ($Y$), and conscientiousness ($U$).
...
The mediation analysis reports a healthy, highly significant $\hat b$ of about 0.5. A referee would read that studying strongly improves scores and that the encouragement works through study time. Every number is precisely estimated, and every causal number is wrong: in this world, forcing students to study more would achieve nothing, since study time does not cause scores. Conscientiousness merely makes the two travel together inside each arm of the experiment. Notice also the estimated direct effect, pushed spuriously *negative*.^[The books must balance. Randomization makes the total effect honestly estimated, so when the confounder inflates the indirect share, the direct share is pushed down to compensate. Analysts then solemnly interpret the negative direct path ("encouragement backfires unless you study"). It is an accounting artifact.] More data would only tighten the intervals around the wrong values.
...
So far, demolition. The constructive turn begins with honesty: draw the confounding into the model. Connect the two disturbances with a curved arrow and give it a parameter, $\rho$, the correlation between everything-else-about-$M$ and everything-else-about-$Y$.
...
Count, as @saris1984 taught. With the direct path included, the unknowns are $a$, $b$, $c$, and $\rho$: four parameters facing three observable correlations. One short; not identified, and this time the counting rule alone convicts. But suppose the theory can honestly assert that there is *no direct path*: $X$ touches $Y$ only through $M$. Then the parameters are three, and the tracing rules give three equations:

$$
\rho_{XM} = a, \qquad
\rho_{MY} = b + a \cdot 0 + \rho\text{-term}, \qquad
\rho_{XY} = a\,b .
$$

The middle equation says the naive mediator-outcome correlation is contaminated: it contains $b$ plus a contribution from the correlated disturbances. But look at the other two. Neither contains $\rho$ at all, because no valid trace from $X$ runs through the curved arrow. Divide:

$$
b = \frac{\rho_{XY}}{\rho_{XM}} .
$$

The causal effect of the mediator is identified *without assuming away the confounding*; $\rho$ has become just another parameter the data estimate. Read the trade in plain terms. We swapped the assumption "no mediator-outcome confounding" for the assumption "no direct effect", which in this role is called the **exclusion restriction**. Neither assumption is testable from these data. The swap is struck entirely on theoretical credit, and when the credit is good, $X$ earns a new job title: an **instrumental variable** [@angrist1996]. We are not interested in $X$ for its own sake; we use its randomness as a lever to wiggle $M$ and watch what happens to $Y$.
...
The estimate lands on the truth: studying does nothing, correctly reported, from confounded data. The second line shows that the whole apparatus reduces to one division.

The exam-style algebra deserves doing once with pure numbers. Suppose we observe $\rho_{XM} = 0.3$, $\rho_{MY} = 0.7$, and $\rho_{XY} = 0.15$. The instrumental-variable estimate is $b = 0.15/0.3 = 0.5$. The naive estimate, the raw mediator-outcome correlation, is $0.7$. The difference, $0.2$, is the confounding that the naive analysis mistook for causation.

Now the fine print, printed large, because each clause is an exam question and a research disaster in waiting.

*Relevance.* The recipe divides by $\rho_{XM} = a$. If the instrument barely moves the mediator, the division amplifies noise; if $a = 0$, there is nothing to divide by at all. Weak instruments do not merely widen intervals; they make the estimator erratic and, when the instrument is even slightly invalid, badly biased [@bound1995].

```{r}
#| label: weak-instrument
#| fig-cap: "Instrumental-variable estimates across 500 simulated studies (n = 1,000, true b = 0). Left: a strong instrument (a = 0.5). Right: a weak one (a = 0.05); same estimator, estimates clipped at plus or minus 10."
one_iv <- function(a) {
...
Sometimes we build the instrument. In an **encouragement design**, we randomize not the behavior itself but an invitation to it, then use the randomized invitation as an instrument for the behavior [@holland1988]. The same logic runs any trial with imperfect compliance: assignment is randomized, treatment received is not, and assignment instruments treatment received. In the JOBS II trial for unemployed workers, only some of those assigned to job-search workshops attended; assignment served as the instrument for participation [@tingley2014].
...
Each example inherits the fine print in local dialect, and the criticisms of instrumental variables are the fine print grown up. Economists learned that celebrated instruments can be weak, with the quarter-of-birth instrument for schooling as the canonical case [@bound1995]. A subtler point is that even a valid instrument answers a narrower question than the one usually asked: it estimates the effect among "compliers", the units the instrument actually moved, and critics like @deaton2010 ask whether that local effect was the question anyone posed. Mediation analysis in psychology has its own bill of indictment: mediators measured minutes before outcomes, assumptions unstated, and a literature of causal-sounding conclusions resting on the naive $b$ path [@bullock2010; @rohrer2022]. The remedy is not to abandon the how-question. It is to treat the mediation model as this book treats every model, as a set of assumptions waiting for a design.

## Modern mediation analysis, briefly

The framework above is linear. Real mediation problems bring binary outcomes, interactions, and nonlinearity, and for those the modern literature defines the effects counterfactually before estimating anything [@vanderweele2016]. Two definitions are worth knowing because they are the workhorses of applied papers. The **natural direct effect** (NDE) compares the outcome under treatment versus control while the mediator is held at the value it would naturally take *under control*: the effect that does not run through the mediator. The **natural indirect effect** (NIE) holds treatment fixed and compares the outcome under the mediator value it would take with treatment versus without: the effect that does run through the mediator. The two add up to the total effect by construction, a decomposition as tidy as the linear one, and in the linear model without interactions the NIE is exactly $ab$. With interactions or a logistic outcome the classic formulas and the counterfactual ones part ways, and the counterfactual ones are right.

The R package `mediation` computes these effects by simulation for a wide menu of models [@tingley2014], under the same no-confounding assumptions we met above (there called "sequential ignorability"). A classic demonstration uses a framing experiment: participants randomized to an anxiety-inducing immigration news story, with anxiety as mediator and sending an anti-immigration letter to Congress as the binary outcome [@brader2008].
...
Identification is a property of the question-design pair, not of the estimator, the sample size, or the software. The mediation story taught each general lesson in miniature. Two opposite models fit identically, so the data cannot orient an edge the design did not orient. A hidden confounder made every estimate precise and wrong, because assumptions the design does not buy fail silently. One untestable assumption, exclusion, bought back identification: assumptions are currency, and the design sets the exchange rate. And the instrument's own fine print moved the limits rather than abolishing them. When you meet an analysis, any analysis, ask the fifth checklist question of @sec-rq-to-interpretation with maximal severity: which feature of the design put the answer to this question into the data at all? If none did, no estimator will get it out. Mediation analysis is an if-then statement: *if* the assumptions hold, *then* this is the mediation process. Papers that claim mediation because the model said so are walking in a circle.
...
A published paper reports, from a cross-sectional survey of 2,000 adolescents: "Social media use predicted depressive symptoms ($\beta = 0.21$). This effect was fully mediated by sleep quality ($ab = 0.19$, bootstrap 95% CI [0.12, 0.27]); the direct effect was not significant. We conclude that social media harms adolescents by disrupting sleep, and that sleep interventions can undo this harm."
...
(a) Correct causal order (use $\rightarrow$ sleep $\rightarrow$ symptoms): unsupported; all three variables were measured at once, and depressed adolescents may sleep badly and scroll more. No use-sleep confounding and no sleep-symptom confounding: unsupported; family conflict, for example, plausibly causes late nights, poor sleep, and distress. Linearity/additivity: conventional. (b) Let family conflict drive social media use, poor sleep, and symptoms, with no arrow from sleep to symptoms at all: the three correlations can reproduce the published coefficients, yet improving sleep changes nothing because sleep never caused the symptoms. (c) Very little. The bootstrap gives an honest interval around $ab$, but the number it brackets is causal only under the assumptions in (a); a precise confidence interval around an unidentified quantity is precision about the wrong thing.
:::
...
(b) The variant also causes facial flushing, which affects social behavior and stress; through which condition does this threaten the analysis, and what is the term of art?
