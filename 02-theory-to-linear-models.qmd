...
(a) $y_1 = 0.6A_1 + 0.4C + 0.693E_1$ and $y_2 = 0.6A_2 + 0.4C + 0.693E_2$. The trait is standardized, so its covariances are correlations.
...
The LISS data cannot be redistributed, so simulate the practice model of @fig-yang instead, with the values of the worked example, $\gamma_1 = -0.4$, $\beta = 0.3$, $\gamma_2 = -0.5$. With `set.seed(2)` and $n = 2{,}000$, draw `xi <- rnorm(n)`, then `eta1 <- -0.4 * xi + rnorm(n, sd = sqrt(1 - 0.16))`, then `eta2 <- 0.3 * eta1 - 0.5 * xi + rnorm(n, sd = sqrt(0.54))`. The disturbance standard deviations have been chosen so that every variable has variance 1; check this with `var()`.
...
eta2 <- 0.3 * eta1 - 0.5 * xi + rnorm(n, sd = sqrt(0.54))
