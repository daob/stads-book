...
Each of the direct effects, indirect effects, spurious relationships, and joint effects makes a specific contribution to the total correlation:
 ...
 * A **joint effect** is a relationship that has been left purposefully unspecified: it could be direct in either direction or spurious -- we just do not know. One way of thinking about joint effects is that they are spurious relationships in which the common cause has been replaced by a curved two-headed arrow. Chains of arrows run from the curve's two ends to the two variables, and the curve's value multiplies in like a coefficient: $X_1 \overset{\phi}{\leftrightarrow} X_2 \overset{b}{\rightarrow} Y$ means the joint effect equals $\phi b$. Either chain may be empty, as it is here on the $X_1$ side: a variable can sit directly on the curve.

@fig-joint draws the situation the fourth rule describes. On the left are three different models of the relationship between $X_1$ and $X_2$: $X_1$ causes $X_2$; $X_2$ causes $X_1$; and the two share an unnamed common cause, drawn as a circle because it is unobserved. Set $\gamma_1 = \gamma_2 = \lambda_1 \lambda_2 = \phi$ and all three imply exactly the same correlations among the observed variables: $\rho_{X_1 X_2} = \phi$, $\rho_{X_2 Y} = \beta$, and $\rho_{X_1 Y} = \phi\beta$. No data on these three variables can tell the stories apart. All that differs is the name of the $\phi\beta$ contribution: an indirect effect in the first story, a spurious relationship in the other two. The single diagram on the right therefore stands for all three at once. It records the covariation as a curved arrow, declines to say which story produced it, and its contribution to $\rho_{X_1 Y}$ is filed under the fourth heading: a joint effect of $\phi\beta$. This is the trade the curved arrow offers. You give up the claim about why $X_1$ and $X_2$ covary, you lose the right to name the contribution, and in exchange the model stops asserting something these data could never check.

![Three stories on the left, one diagram on the right. Whichever of the three models is true, the implied correlations among $X_1$, $X_2$, and $Y$ are identical once $\phi = \gamma_1 = \gamma_2 = \lambda_1\lambda_2$, so the curved arrow can stand for all of them. Its contribution to the correlation between $X_1$ and $Y$ is the joint effect $\phi\beta$.](diagrams/joint-effect.png){#fig-joint width="90%" fig-align="center"}

One more remark makes these four rules a complete recipe: the sum runs over *every* contribution of *every* kind. Every arrow between the pair, every one-way chain, every common cause with every pair of chains running out of it, every curved arrow with every pair of chains running out of its ends; with longer chains, multiply every coefficient along the way. 
...
Between $S$ and $I$ only one of the four shapes can be built: the direct arrow, so $\rho_{SI} = a = -0.20$. You might be tempted by the route $S \rightarrow R \leftarrow I$, but no rule produces it: $R$ is a common effect of the two, not a common cause.

Between $S$ and $R$ two shapes appear: the direct effect $c$, and an indirect effect through inflammation, the chain $S \rightarrow I \rightarrow R$ with product $ab$.
...
Decomposing a correlation is bookkeeping. Take the pair of variables and walk the four rules in order: the direct arrow, if there is one; every one-way chain from one to the other; every common cause, with each pair of chains running out of it; every curved arrow, likewise. Add up what you find. In any model you can draw, the lists are short, and together they are the whole correlation.

::: {.callout-tip icon="false" title="Exercise 2.4"}
Practice the mechanics one contribution at a time, on @fig-yang with the same values.
...
(b) $R$ is a common effect of the two, and none of the four shapes contains a common effect: it is not an arrow between them, not a one-way chain, not a common cause, not a curved arrow. Common effects do not induce correlation between their causes. (They do induce one *among units selected on $R$*, a separate trap with its own name, collider bias.)
...
The 0.5 on the curved arrow was not estimated from these twins; genetics fixes it. Decompose the covariance between $y_1$ and $y_2$. The shared environment $C$ is a common cause, with a chain of 0.5 to each twin: a spurious relationship of $0.5 \times 0.5 = 0.25$. The genetic factors are joined by the curved arrow, with the chain 0.7 out of each end: a joint effect of $0.7 \times 0.5 \times 0.7 = 0.245$. The unique environments contribute nothing, since each acts on one twin alone: they are not common causes, and no chain connects them to the other twin. The sum is $0.495$.^[Strictly this is the implied covariance. The implied variance of each twin's score is $0.49 + 0.25 + 0.25 = 0.99$, so the implied correlation is $0.495/0.99 = 0.50$. The decomposition gives correlations when the variables are standardized; here they very nearly are, and the gap is 1 percent. With larger loadings it would be larger.]
