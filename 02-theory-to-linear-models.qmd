 ...
 * A joint effect is a spurious relationship in which the common cause has been replaced by a curved two-headed arrow. Chains of arrows run from the curve's two ends to the two variables, and the curve's value multiplies in like a coefficient: $X_1 \overset{\phi}{\leftrightarrow} X_2 \overset{b}{\rightarrow} Y$ means the joint effect equals $\phi b$. Either chain may be empty, as it is here on the $X_1$ side: a variable can sit directly on the curve.

Two remarks make these four rules a complete recipe.^[The recipe works for **recursive** models, meaning models without reciprocal effects or feedback loops; with arrows running both ways the implied correlations have to be derived algebraically. This way of computing implied correlations is classically known as the *tracing rules* [@wright1934; @saris1984].] First, the sum runs over *every* contribution of *every* kind: every arrow between the pair, every one-way chain, every common cause with every pair of chains running out of it, every curved arrow with every pair of chains running out of its ends; with longer chains, multiply every coefficient along the way. Second, nothing else counts. None of the four shapes contains two arrows colliding head-on, so a common *effect* of the pair never contributes: two causes of the same thing need not be correlated at all. As for the name of the fourth kind, it records an admission. The curved arrow is a common cause the model declined to name, or an effect it declined to orient, so its contribution can be filed neither under indirect nor under spurious, and it gets a heading of its own.
