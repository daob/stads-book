...
This is where our choice for linear and additive models helps. Linear and additive equations can only affect the means, variances, and covariances of variables. Any other features of the data distribution have nothing to do with the parameters of interest (except through means, variances, and covariances). This simplifies the task: we only need to work out how the parameters relate to the means, variances, and covariances, and everything else follows.
...
There are three equations and three unknowns. The first gives $\gamma_1$ directly. Substituting it into the other two leaves two linear equations in $\beta$ and $\gamma_2$, and solving them gives
...
I am using examples of relatively simple models for didactic purposes. Simple models can be very useful in practice. But models do not have to be simple. As an illustration, see the model below for health outcomes of preterm babies shared by Fustolo-Gunnink et al. 

![Example of causal loop diagram as developed for cord blood derived red blood cell transfusions (Fustolo-Gunnink et al. 2026).](images/causal-loop.jpg){#causal-loop width="100%" fig-align="center"}
