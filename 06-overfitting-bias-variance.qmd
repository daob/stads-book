...
<!-- DO: rewrite below as follows. First derive that "the best possible predictor from the observables is $E(Y \mid X_1, X_2) = \rho_{UX}^2\,X_1 + b\,X_2$", and that Var(Y|X1,X2) = rho^2 + var(eps). Then just give the irreducible variance by substituting 0.9 and 1. For bias, say "the average fit is E(), so the bias is E(Y | X_1 X_2) - E() = .. = .. For variance, leave as-is. -->
...
<!-- DO: rewrite the sentence below in normal language. I have no idea what you mean. -->
The table shows the whole mechanism: a tiny bias accepted, a coefficient's rent saved, and a true cause whose effect is too small, in twenty observations, to pay its own way.
